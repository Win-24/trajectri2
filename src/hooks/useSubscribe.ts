...
export const UseSubscribe = ({channelId}: UseSubscribeProps) => {
    ...
    const toggleSubscribed = useCallback(async () => {
        ...
        try {
            if (hasSubscribed) {
                await axios.delete("/api/users/subscriptions", {data: {
                    channelId
                }})
            } else {
            ...
            router.refresh()
            toast.success(hasSubscribed ? "Unsubscribed successfully" : "Subscribed successfully")
        } catch (error) {
            toast.error(
    ...
    return {
        hasSubscribed, toggleSubscribed
    }
};
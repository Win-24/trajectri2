...
const VideoCard: React.FC<VideoCardProps> = ({
  ...
  return (
    <Link className="w-full" href={`/video/${video.id}`}>
        <div className={`flex items-start ${isVertical ? "flex-col" : "flex-row"} gap-2 cursor-pointer overflow-hidden`}>
            ...
            <div className={`flex gap-2 items-start ${isVertical ? "w-full" : "w-3/5"}`}>
                {
                    channel && 
                    channelAvatar && 
                    isVertical ? (<Avatar className="mt-1" imageSrc={channel.imageSrc} />) : null
                }

                <div className="flex flex-col">
                    <h3 className={`line-clamp-2 ${isVertical ? "text-lg" : "text-md leading-5"}`}>
                        {video.title}
                    </h3>
